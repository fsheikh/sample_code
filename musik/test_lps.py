...
import os
import logging
from classify import consecutive_detected_frames
...
if __name__ == "__main__":
    ...
    longestSeq = consecutive_detected_frames(testVector, 1.0)
    if longestSeq != 3:
        logger.error("Test case 1 failed")
        raise ValueError
    longestSeq = consecutive_detected_frames(np.zeros(5), 1.0 )
    if longestSeq != 0:
        logger.error("Test case 2 failed")
        raise ValueError
    longestSeq = consecutive_detected_frames(np.ones(5), 1.0)
    if longestSeq != 5:
        logger.error("Test case 3 failed")
        raise ValueError
    testVector = np.array([48.0, 47.0, -1.0, 5.0, 0.0, 5.0, 5.0, 99.9, 77.7])
    longestSeq = consecutive_detected_frames(testVector, 5.0)
    if longestSeq != 2:
